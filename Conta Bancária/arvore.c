...
void pre_ordem_recursao(NODE *raiz) {
    if (raiz != NULL) {
        if(item_get_chave(raiz->item) < pow(10,10))
        {
            printf("0");
        }
...
void operacoes_bancarias()
{
    ...
    char c;
    char *cpf_busca;
    long long int cpf_busca_int;

    arvoreBinaria = arvore_binaria_criar();
    scanf("%d ", &qtd_contas);
    inserir_items(arvoreBinaria, qtd_contas);
    scanf("%c ", &c);

    if(c == 'B')
    {
        cpf_busca = readLine();
        cpf_busca_int = ler_cpf(cpf_busca);
        item_imprimir(abb_busca(arvoreBinaria, cpf_busca_int));
        printf("\n");
        free(cpf_busca);
    }
    else if (c == 'I')
    {
        ...
        printf("Preorder\n");
        abb_inserir(arvoreBinaria, item);
    ...
    else if (c == 'R')
    {
        ...
        cpf_busca_int = ler_cpf(cpf_busca);
        item_imprimir(abb_busca(arvoreBinaria, cpf_busca_int));
        arvore_binaria_remover(arvoreBinaria, cpf_busca_int);
        printf("Preorder\n");
        arvore_binaria_pre_ordem(arvoreBinaria);
        printf("\n");
        free(cpf_busca);
    ...
    ab_apagar_arvore(&arvoreBinaria);
}
